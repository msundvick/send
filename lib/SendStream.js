...
SendStream.prototype.sendFile = function sendFile (path) {
  ...
  fs.stat(path, function onstat (err, stat) {
    if ((err && err.code === 'ENOENT' || stat && stat.isDirectory()) && !extname(path) && path[path.length - 1] !== sep) {
      // not found, check extensions
      return next(err, stat ? stat.isDirectory() : false)
    }
  ...
  function next (err, wasDir) {
    if (self._extensions.length <= i) {
      if (wasDir) return self.redirect(path)
      return err
    ...
    fs.stat(p, function (err, stat) {
      if (err) return next(err, wasDir)
      if (stat.isDirectory()) return next(null, wasDir)
